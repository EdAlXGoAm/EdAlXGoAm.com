...
const CostillasBbq = ({ index, comanda, platillo, platillo_espacios, toggleChecked_Status, setToggleChecked_Status, propiedadesComanda, handleUpdatePropiedadesComanda}) => {
    const costillasOptions = [
        'Orden de 2',
    ];
    const aderezos = [
        'BBQ',
    ...
    const [selectedCostillasBbq, setselectedCostillasBbq] = useState(propiedadesComanda.selectedCostillasBbq);

    const handleDropdownChangeCostillasBbq = (e) => {
        const newPropiedades = propiedadesComanda;
        newPropiedades.precio = e.precio;
        newPropiedades.selectedCostillasBbq = e.value;
        handleUpdatePropiedadesComanda(newPropiedades);
    };
    ...
    return (
        <div>
            <div>
                <DropDown opciones_in={costillasOptions} selectedValue={selectedCostillasBbq} onDropdownChange={handleDropdownChangeCostillasBbq} precios={[95]}
                                                                                                                                    precios_papas={[110]} hide_show_toggle={hide_show_toggle} setToggleChecked={setToggleChecked}/>
            </div>
